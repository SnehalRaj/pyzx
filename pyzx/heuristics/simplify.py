...
def apply_best_match(g, lcomp_matches, pivot_matches):
    lcomp_matches.sort(key= lambda m: m[2],reverse=False)
    pivot_matches.sort(key= lambda m: m[2],reverse=False)
    method = "pivot"

    if len(lcomp_matches) > 0:
        if len(pivot_matches) > 0:
            if lcomp_matches[0][2] > pivot_matches[0][2]:
                method = "lcomp"      
        else:
            method = "lcomp"
    else:
...
def simulated_annealing_reduce(g: BaseGraph[VT,ET], iterations=100, alpha=0.95, cap=-100000):
    ...
    while temperature > epsilon:
        ...
        if method == "pivot":
            ...
            apply_lcomp(g,match[1])
        # print(f'Iteration {it} Method {method} temperature {temperature}')
        temperature *=alpha
